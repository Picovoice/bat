...
void print_usage(const char *program_name) {
    fprintf(stderr,
            "Usage : %s -a ACCESS_KEY -l LIBRARY_PATH -m MODEL_PATH [-y DEVICE] [-v VOICE_THRESHOLD] wav_path0 wav_path1 ...\n"
...
int picovoice_main(int argc, char **argv) {
    ...
    int opt;
    while ((opt = getopt(argc, argv, "a:m:l:e:y:v:i")) != -1) {
        switch (opt) {
            case 'a':
                access_key = optarg;
                break;
    ...
    const size_t frame_length = (size_t) pv_bat_frame_length_func();
    ...
    for (int32_t i = optind; i < argc; i++) {
        ...
        while ((int32_t) drwav_read_pcm_frames_s16(&f, frame_length, pcm) == frame_length) {
            ...
            if (scores) {
                ...
                for (int32_t j = 0; j < PV_BAT_LANGUAGE_NUM_LANGUAGES; j++) {
