...
INCLUDE_FILES = ('../../LICENSE', '__init__.py', '_factory.py', '_bat.py', '_util.py')
INCLUDE_LIBS = ('linux', 'mac', 'windows', 'raspberry-pi')

os.system('git clean -dfx')

package_folder = os.path.join(os.path.dirname(__file__), 'pvbat')
os.mkdir(package_folder)
manifest_in = ""

for rel_path in INCLUDE_FILES:
    shutil.copy(os.path.join(os.path.dirname(__file__), rel_path), package_folder)
    manifest_in += "include pvbat/%s\n" % os.path.basename(rel_path)

model_file = 'lib/common/bat_params.pv'
...
for platform in INCLUDE_LIBS:
    ...
    manifest_in += "recursive-include pvbat/lib/%s *\n" % platform
