...
from _bat import Bat
from _util import *


class BatPerformanceTestCase(unittest.TestCase):
    ...
    def test_performance_proc(self):
        ...
        num_frames = len(pcm) // bat.frame_length

        perf_results = list()
        for i in range(self.NUM_TEST_ITERATIONS + 1):
            total_proc_time = 0
            for j in range(num_frames):
                frame = pcm[j * bat.frame_length:(j + 1) * bat.frame_length]
                start = time.perf_counter()
                bat.process(frame)
...
if __name__ == '__main__':
    if len(sys.argv) != 6:
        print("Usage: python test_bat_perf.py <ACCESS_KEY> <NUM_TEST_ITERATIONS> "
