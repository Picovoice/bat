...
def default_library_path(relative):
    if platform.system() == 'Darwin':
        if platform.machine() == 'x86_64':
            return os.path.join(os.path.dirname(__file__), relative, 'lib/mac/x86_64/libpv_bat.dylib')
        elif platform.machine() == "arm64":
            return os.path.join(os.path.dirname(__file__), relative, 'lib/mac/arm64/libpv_bat.dylib')
    elif platform.system() == 'Linux':
        linux_machine = _linux_machine()
        if linux_machine == 'x86_64':
            return os.path.join(os.path.dirname(__file__), relative, 'lib/linux/x86_64/libpv_bat.so')
        elif linux_machine in _RASPBERRY_PI_MACHINES:
            return os.path.join(
                os.path.dirname(__file__),
                relative,
                'lib/raspberry-pi/%s/libpv_bat.so' % linux_machine)
    elif platform.system() == 'Windows':
        if platform.machine().lower() == 'amd64':
            return os.path.join(os.path.dirname(__file__), relative, 'lib/windows/amd64/libpv_bat.dll')
        elif platform.machine().lower() == 'arm64':
            return os.path.join(os.path.dirname(__file__), relative, 'lib/windows/arm64/libpv_bat.dll')

    raise NotImplementedError('Unsupported platform.')


def default_model_path(relative):
    return os.path.join(os.path.dirname(__file__), relative, 'lib/common/bat_params.pv')
