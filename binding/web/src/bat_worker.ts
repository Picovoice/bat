...
import PvWorker from 'web-worker:./bat_worker_handler.ts';
...
export class BatWorker {
   ...
   *
   * @returns An instance of BatWorker.
   */
  public static async create(
    ...
    const returnPromise: Promise<BatWorker> = new Promise((resolve, reject) => {
      // @ts-ignore - block from GC
      this.worker = worker;
      worker.onmessage = (event: MessageEvent<BatWorkerInitResponse>): void => {
        switch (event.data.command) {
          case 'ok':
            worker.onmessage = (ev: MessageEvent<BatWorkerProcessResponse>): void => {
              switch (ev.data.command) {
                case 'ok':
                  scoresCallback(ev.data.scores);
                  break;
                case 'failed':
                case 'error': {
            ...
            resolve(new BatWorker(worker, event.data.version, event.data.frameLength, event.data.sampleRate));
            break;
          case 'failed':
          case 'error': {
   ...
   * The scores result will be supplied with the callback provided when initializing the worker either
  ...
  public release(): Promise<void> {
    const returnPromise: Promise<void> = new Promise((resolve, reject) => {
      this._worker.onmessage = (event: MessageEvent<BatWorkerReleaseResponse>): void => {
        switch (event.data.command) {
          case 'ok':
            resolve();
            break;
